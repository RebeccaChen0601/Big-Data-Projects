...
import org.apache.spark.streaming.{ManualClockWrapper, Seconds, Minutes, StreamingContext}
...
object TrendingArrivals {
  ...
  def main(argv: Array[String]): Unit = {
    ...
    val updateState = (batchTime: Time, key: String, value: Option[Int], state: State[Int]) => {
        val curr = value.getOrElse(0)
        val prev = state.getOption.getOrElse(0)
        if (curr >= prev * 2 && curr >= 10) {
            if (key == "goldman") {
                println("Number of arrivals to Goldman Sachs has doubled from " +prev.toString + " to " + curr.toString + " at " + batchTime.milliseconds.toString + "!")
            } else {
                println("Number of arrivals to Citigroup has doubled from " +prev.toString + " to " + curr.toString + " at " + batchTime.milliseconds.toString + "!")
            }
        }
        state.update(curr)
        Some((key, (curr, batchTime.milliseconds, prev))) 
    }
