...
public class RunPersonalizedPageRankBasic extends Configured implements Tool {
    ...
    private static class MapClass extends
        ...
        private static int num_sources = 0;
        ...
        public void setup(Mapper<IntWritable, PageRankNode, IntWritable, PageRankNode>.Context context) {
            num_sources = context.getConfiguration().getInt("num_sources", 1);
        }
        ...
        public void map(IntWritable nid, PageRankNode node, Context context)
            ...
            if (node.getAdjacencyList().size() > 0) {
                ...
                // build the list of PageRank mass for each neighbour
                float[] mass = new float[num_sources];
...
//        }
    ...
    private static class MapPageRankMassDistributionClass extends
        ...
        public void setup(Context context) throws IOException {
            Configuration conf = context.getConfiguration();
            String [] missMassOrig = conf.getStrings("MissingMass", "");
            String [] sourcesOrig = conf.getStrings("Sources", "");
            for (int i = 0; i < missMassOrig.length; i++) {
                missingMass.add(Float.parseFloat(missMassOrig[i]));
            }
            for (int i = 0; i< sourcesOrig.length; i++) {
                sources.add(Integer.parseInt(sourcesOrig[i]));
            }
    ...
    public int run(String[] args) throws Exception {
        ...
        int num_sources = sources.split(",").length;
        ...
        LOG.info(" - souces: " + sources);
        LOG.info(" - number of source: " + num_sources);

        // Iterate PageRank.
        for (int i = s; i < e; i++) {
            iteratePageRank(i, i + 1, basePath, n, sources, num_sources);
        }
    ...
    private void iteratePageRank(int i, int j, String basePath, int numNodes,
                                 String sources, int num_sources) throws Exception {

        // Each iteration consists of two phases (two MapReduce jobs).

        // Job 1: distribute PageRank mass along outgoing edges.
        ArrayList<Float> mass = phase1(i, j, basePath, numNodes, sources, num_sources);

        // Find out how much PageRank mass got lost at the dangling nodes.
        ArrayList<Float> missing = new ArrayList<Float>();
        for (int a = 0; a < mass.size(); a++) {
            float missing_mass = 1.0f - (float) StrictMath.exp(mass.get(a));     ///////////?????????????????/
            missing.add(missing_mass > 0? missing_mass : 0.0f);
        }
    ...
    private ArrayList<Float> phase1(int i, int j, String basePath, int numNodes,
        ...
        job.getConfiguration().setInt("num_sources", num_sources);
        ...
        for (FileStatus f : fs.listStatus(new Path(outm))) {
            ...
            for (int a = 0; a < num_sources; a++) {
                mass.set(a, sumLogProbs(mass.get(a), fin.readFloat()));
            }
            fin.close();
        }
    ...
    private void phase2(int i, int j, ArrayList<Float> missing, String basePath, int numNodes, String sources) throws Exception {
        ...
        for (int a = 0; a < missing.size(); a++) {
            missMassStr += missing.get(a).toString() + ", ";
        }
        missMassStr = missMassStr.substring(0, missMassStr.length()-2);

        LOG.info("Missing masses: " + missMassStr);
        ...
        job.getConfiguration().setBoolean("mapred.map.tasks.speculative.execution", false);
        job.getConfiguration().setBoolean("mapred.reduce.tasks.speculative.execution", false);
        job.getConfiguration().setStrings("MissingMass", missMassStr);
        job.getConfiguration().setInt("NodeCount", numNodes);
