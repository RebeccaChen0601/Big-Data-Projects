...
public class ExtractTopPersonalizedPageRankNodes extends Configured implements Tool {
    ...
    private static class MyMapper extends
        ...
        public void setup(Context context) throws IOException {
            ...
            queue = new TopScoredObjects[num_sources];
    ...
    private static class MyReducer extends
        ...
        public void setup(Context context) throws IOException {
            int k = context.getConfiguration().getInt("n", 100);
            num_sources = context.getConfiguration().getInt("NUM_SOURCES", 1);
            String[] source_str = context.getConfiguration().getStrings("SOURCE_NODES", "");
            queue = new TopScoredObjects[num_sources];
            for (int i = 0; i < num_sources; i++) {
                sources.add(Integer.parseInt(source_str[i]));
                queue[i] = new TopScoredObjects<>(k);
            }
        }
        ...
        public void reduce(PairOfInts nid, Iterable<FloatWritable> iterable, Context context)
            ...
            queue[nid.getLeftElement()].add(nid.getRightElement(), iter.next().get());
        ...
        public void cleanup(Context context) throws IOException, InterruptedException {
            ...
            for (int i = 0; i < num_sources; i++) {
                ...
                for (PairOfObjectFloat<Integer> pair : queue[i].extractAll()) {
                    ...
                    value.set(String.format("%.5f",  (float)StrictMath.exp(pair.getRightElement())));
    ...
    public int run(String[] args) throws Exception {
        ...
        conf.setInt("NUM_SOURCES", num_sources);
